...
import math
__all__ = [
    'VGG', 'vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg13_bn_3neurons', 'vgg16', 'vgg16_bn',
    'vgg19_bn', 'vgg19'
]
...
class VGG(nn.Module):
    ...
    def __init__(self, cfg, batch_norm=False, num_classes=1000, embed=False, img_size=32):
        ...
        n_feat = 512
        if embed:
            self.embed = nn.Linear(n_feat, 3)
            n_feat = 3

        self.classifier = nn.Linear(n_feat, num_classes)
    ...
    def forward(self, x, is_feat=False, preact=False):
        h = x.shape[2]
        x = F.relu(self.block0(x)); f0 = x # 32
        x = self.pool0(x) # 16
...
def vgg8_bn_3neurons(**kwargs):
    """VGG 8-layer model (configuration "S")
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = VGG(cfg['S'], batch_norm=True, embed=True, **kwargs)
    return model
...
def vgg13_bn_3neurons(**kwargs):
    """VGG 13-layer model (configuration "B") with batch normalization"""
    model = VGG(cfg['B'], batch_norm=True, embed=True, **kwargs)
    return model

...
def count_nonzero_weights(model):
    """
    Count the number of non-zero weights in the model's conv and fc layers.

    Args:
    - model (nn.Module): The model to count non-zero weights in.

    Returns:
    - total_weights (int): The total number of weights in the model.
    - total_nonzero (int): The total number of non-zero weights in the model.
    """
    total_weights = 0
    total_nonzero = 0
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
            ...
            total_weights += weight_tensor.numel()  # Total number of weights
            total_nonzero += torch.count_nonzero(weight_tensor).item()  # Count of non-zero weights
    
    return total_weights, total_nonzero

def unstructured_pruning(model, pruning_percentage):
    """
    Apply unstructured pruning to conv and fc layers in the given model.

    Args:
    - model (nn.Module): The model to be pruned.
    - pruning_percentage (float): The percentage of individual weights to prune.
    """
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
